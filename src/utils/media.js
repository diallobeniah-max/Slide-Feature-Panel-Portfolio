...
async function recordMediaRange(asset, options, onProgress) {
  const outputKind =
    options.outputMode === "audio" || asset.type === "audio"
      ? "audio"
      : "video";
  ...
  // No crossOrigin — object URLs are same-origin and crossOrigin breaks them
  source.preload = "auto";
  ...
  const end = Math.max(
    start + 0.1,
    Math.min(asset.trimEnd || duration, duration),
  );
  const captureLength = Math.max(0.1, end - start);
  const highQuality = options.quality >= 90;
  const preset =
    QUALITY_PRESETS.find((item) => item.quality === options.quality) ||
    QUALITY_PRESETS[1];
  ...
  const recorder = new MediaRecorder(stream, {
    ...
    videoBitsPerSecond:
      outputKind === "video"
        ? highQuality
          ? Math.max(preset.videoBitsPerSecond, 12_000_000)
          : preset.videoBitsPerSecond
        : undefined,
    audioBitsPerSecond: highQuality
      ? Math.max(preset.audioBitsPerSecond, 256_000)
      : preset.audioBitsPerSecond,
  });
  ...
  return {
    blob: new Blob(chunks, {
      type: mimeType || (outputKind === "audio" ? "audio/webm" : "video/webm"),
    }),
    mimeType,
    duration,
  };
...
async function optimizeImage(asset, options, onProgress) {
  ...
  const quality =
    options.quality >= 90 ? 1 : Math.max(0.5, options.quality / 100);
